<template>
  <div>
    ...
    <v-skeleton-loader class="user-card" v-if="loading"
      type="card-heading, list-item-avatar-two-line, list-item-three-line, actions"></v-skeleton-loader>
    <v-card class="user-card" v-if="!loading && isLogin" :loading="loading">
      <v-card-title> 个人中心 </v-card-title>
      <v-card-text>
        <v-row style="margin-left: 0px" class="mt-2">
          <v-avatar class="mr-2" color="grey darken-1" size="60"><img :src="userInfo.avatar" alt="用户头像" /></v-avatar>
          <v-col cols="13" style="margin-top: -10px" class="user-info-col">
            <p>
              {{ userInfo.name }}
              <v-chip v-if="userInfo.isAdmin" class="ma-1" :color="userInfo.isAdmin ? adminBadgeColor : 'grey'"
        ...
        <v-col class="ml-0 mt-2 user-info-col">
          ...
          <v-progress-linear class="mt-5" :value="userInfo.exp" rounded height="25px" color="yellow">
